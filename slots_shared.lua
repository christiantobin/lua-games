...
local function checkWin(reel, lines, betAmount)
	local wins = 0
	local winLines = {}
	...
	for line = 1, lines do
		...
		if count >= 3 then
			...
			totalPayout = totalPayout + PAYTABLE[symbol][math.min(count, 5) - 2] * betAmount
...
local function playFreeGames(money, lines, betAmount)
	...
	while freeGamesRemaining > 0 do
		...
		local wins, winLines, _, miniPrize, minorPrize, majorPrize, totalPayout = checkWin(reel, lines, betAmount)
		if wins > 0 then
...
return {
	...
	printReel = printReel,
	getLineSymbols = getLineSymbols,
	checkWin = checkWin,
	checkRetrigger = checkRetrigger,
	playFreeGames = playFreeGames,
}
